...
BOOL CScintillaWnd::OpenFile(LPCTSTR lpFileName)
{
	...
	{//utf16
		CDuiStringW strBuf((LPCWSTR)(pbuf+2),dwSize/2-1);
		str=DUI_CW2A(strBuf,CP_UTF8);
	}else if(dwSize>3 && pbuf[0]==0xEF && pbuf[1]==0xBB && pbuf[2]==0xBF)
	{//utf8
		str=CDuiStringA((LPCSTR)(pbuf+3),dwSize-3);
	}else
	{//ansi
		CDuiStringA strAnsi((LPCSTR)pbuf,dwSize);
		CDuiStringW strW=DUI_CA2W(strAnsi,CP_ACP);
		str=DUI_CW2A(strW,CP_UTF8);
	}
